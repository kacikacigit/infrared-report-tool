...
本脚本假设以下三样东西/Users/zhanganan/Desktop/人工智能/opencode项目集/00-虚拟环境/提取数值虚拟环境/ocr_venv/bin/和本脚本放在【同一个文件夹】里（不需要修改任何路径配置）：
    ./图谱/                图片文件夹（红外测温图片）
...
def interactive_review_gui(data_rows_info, surface_temp, debug_file_map):
    ...
    DEFAULT_HINT = "↑ = 上一条    |    回车 = 确认并下一条    |    看不清就直接输入正确数值后回车"
    ...
    def try_save_current():
        """把输入框里的内容保存到当前行（回车确认/上一条时都会调用）。
        输入为空则保留原值；输入了但不是合法数字则拒绝并提示，不允许离开当前行。"""
        idx = state['index']
        info = data_rows_info[idx]
        r = info['row']
