...
【整体流程】
    1. 识别：分别框选"Ar1温度"和"拍摄日期+拍摄时间"两个区域（各框一次，所有图片复用），
       对图谱文件夹里的每张图片（按文件名顺序，依次对应Excel第2、3、4...行）自动识别。
    2. 人工核对：弹出一个界面，每一行同时显示"Ar1裁剪图+温度输入框"和
       "日期时间裁剪图+日期/时间输入框"，回车确认并下一条，↑回到上一条，可随时跳过剩余核对。
...
# 填入报告的新数据统一使用的字体（只针对脚本新填入的内容，模板原有文字不动）
CHINESE_FONT = "宋体"
...
def parse_date_time(text):
    """从OCR文本中解析出日期(date对象)和时间(HH:MM:SS字符串)，任一部分解析失败则返回None。"""
    date_val = None
    time_val = None
    if text:
...
def ocr_datetime(image_path, roi_ratio):
    ...
    for variant in variants:
        for cfg in configs:
            text = pytesseract.image_to_string(variant, config=cfg)
            raw_texts.append(text.strip())
            date_val, time_val = parse_date_time(text)
...
def interactive_review_gui(data_rows_info, ar1_results, datetime_results, ar1_debug_map, dt_debug_map):
    """
    单窗口人工核对：每一行同时显示Ar1裁剪图+温度输入框、日期时间裁剪图+日期/时间输入框。
    ...
    try:
        ...
        return interactive_review_cli(data_rows_info, ar1_results, datetime_results, ar1_debug_map, dt_debug_map)

    ar1_reviewed = dict(ar1_results)
    dt_reviewed = dict(datetime_results)  # {row: (date_str_or_None, time_str_or_None)}
    total = len(data_rows_info)
    state = {'index': 0}
    photo_refs = {'ar1': None, 'dt': None}
    prev_btn_ref = {}
    ...
    # --- Ar1 区域 ---
    tk.Label(root, text="Ar1 温度", font=("PingFang SC", 11, "bold")).pack()
    ar1_img_label = tk.Label(root)
    ar1_img_label.pack(pady=4)
    ar1_var = tk.StringVar()
    ar1_entry = tk.Entry(root, textvariable=ar1_var, font=("PingFang SC", 18), justify="center", width=12)
    ar1_entry.pack(pady=4)

    # --- 日期时间区域 ---
    tk.Label(root, text="拍摄日期 / 拍摄时间", font=("PingFang SC", 11, "bold")).pack(pady=(10, 0))
    dt_img_label = tk.Label(root)
    dt_img_label.pack(pady=4)
    dt_frame = tk.Frame(root)
    dt_frame.pack(pady=4)
    tk.Label(dt_frame, text="日期(YYYY-MM-DD):").grid(row=0, column=0, padx=4)
    date_var = tk.StringVar()
    date_entry = tk.Entry(dt_frame, textvariable=date_var, font=("PingFang SC", 14), width=12)
    date_entry.grid(row=0, column=1, padx=4)
    tk.Label(dt_frame, text="时间(HH:MM:SS):").grid(row=0, column=2, padx=4)
    time_var = tk.StringVar()
    time_entry = tk.Entry(dt_frame, textvariable=time_var, font=("PingFang SC", 14), width=10)
    time_entry.grid(row=0, column=3, padx=4)
    ...
    def load_image_into(label, path, ref_key, max_w=500, max_h=260):
    ...
    def load_row(idx):
        ...
        date_current, time_current = dt_reviewed.get(r, (None, None))
        mark = ""
        if ar1_current is None or date_current is None or time_current is None:
            mark = "   ⚠️ 有内容识别失败，需手动输入"

        info_label.config(
            text=f"[{idx + 1}/{total}]  设备: {info['device_name']}{mark}"
        )

        load_image_into(ar1_img_label, ar1_debug_map.get(r), 'ar1')
        load_image_into(dt_img_label, dt_debug_map.get(r), 'dt')
    ...
    def try_save_current():
        ...
        if ar1_val != "":
            try:
                ...
                hint_label.config(text="⚠️ Ar1温度不是有效数字，请重新输入！", fg="red")
                return False

        date_val_str = date_var.get().strip()
        time_val_str = time_var.get().strip()
        parsed_date = dt_reviewed.get(r, (None, None))[0]
        parsed_time = dt_reviewed.get(r, (None, None))[1]

        if date_val_str != "":
            try:
                parsed_date = datetime.date.fromisoformat(date_val_str)
            except ValueError:
        ...
        if time_val_str != "":
            if not re.match(r'^\d{1,2}:\d{2}:\d{2}$', time_val_str):
                ...
                return False
            parsed_time = time_val_str

        dt_reviewed[r] = (parsed_date, parsed_time)
        return True
    ...
    return ar1_reviewed, dt_reviewed


def interactive_review_cli(data_rows_info, ar1_results, datetime_results, ar1_debug_map, dt_debug_map):
    """命令行版兜底核对方案。"""
    ar1_reviewed = dict(ar1_results)
    dt_reviewed = dict(datetime_results)
    total = len(data_rows_info)
    ...
    for i, info in enumerate(data_rows_info, start=1):
        ...
        print(f"  当前 Ar1温度={ar1_current}  拍摄日期={date_current}  拍摄时间={time_current}")
        ...
        while True:
            val = input("  Ar1温度 (回车=保留当前值): ").strip()
        ...
        while True:
            ...
            if val == "":
                break
            try:
                date_current = datetime.date.fromisoformat(val)
                break
            except ValueError:
        ...
        dt_reviewed[r] = (date_current, time_current)
...
def step1_recognize_and_review():
    ...
    ar1_roi = get_roi(sample_path, AR1_ROI_CONFIG_FILE, "Ar1温度数值", "框选Ar1温度区域 (Space/Enter确认, C取消)")
    dt_roi = get_roi(sample_path, DATETIME_ROI_CONFIG_FILE, "拍摄日期+时间",
                      "框选拍摄日期+时间区域，两者要一起框进去 (Space/Enter确认, C取消)")
    ...
    for i, info in enumerate(data_rows_info, start=1):
        ...
        date_val, time_val, dt_crop, dt_raw = ocr_datetime(img_path, dt_roi)

        ar1_results[r] = ar1_val
        datetime_results[r] = (date_val, time_val)

        status = f"Ar1={ar1_val if ar1_val is not None else NOT_RECOGNIZED_TEXT}  " \
    ...
    fail_count = sum(
        ...
        if ar1_results.get(info['row']) is None or datetime_results.get(info['row'], (None, None))[0] is None
        or datetime_results.get(info['row'], (None, None))[1] is None
    )
    ...
    if do_review:
        ar1_results, datetime_results = interactive_review_gui(
            data_rows_info, ar1_results, datetime_results, ar1_debug_map, dt_debug_map
        )
    ...
    for info in data_rows_info:
        ...
        date_val, time_val = datetime_results.get(r, (None, None))
    ...
    for info in data_rows_info:
        ...
        row_data['date'], row_data['time'] = datetime_results.get(r, (None, None))
...
def set_cell_text(cell, text):
    """清空单元格已有内容，写入新文本，并对新写入的文字应用统一字体（宋体/Times New Roman/8.5号）。"""
