# -*- coding: utf-8 -*-
"""
91号文《红外精确测温报告》自动生成程序 V3
======================================================

V3 新增功能：
  1. Step1 时间标框改为可选：部分测温图片上无时间信息，
     标完 Ar1 温度后可选择跳过日期时间的框选与 OCR
  2. 跳过时间 OCR 时，Excel 日期/时间列自动填入"无时间信息"
  3. 跳过时间 OCR 时，核对环节只核对 Ar1 温度，不弹时间窗口

V2 已有功能（全部保留）：
  - 分步执行：--step1 / --step2 / 交互式菜单
  - 防覆盖机制：自动编号后缀 / --suffix 手动指定
  - step2 直接从已有 Excel 生成 Word，不动 OCR

使用方法：
  python3 91号文报告自动生成-终端版V3.py              # 完整运行（step1 + step2）
  python3 91号文报告自动生成-终端版V3.py --step1       # 仅运行 step1（识别 + 核对 + 写Excel）
  python3 91号文报告自动生成-终端版V3.py --step2       # 仅运行 step2（读取已有Excel，生成Word）
  python3 91号文报告自动生成-终端版V3.py --step2 --suffix v2   # step2 + 输出文件名加_v2
  python3 91号文报告自动生成-终端版V3.py --cli         # 命令行核对模式（避免macOS tkinter崩溃）

文件结构（与脚本同目录）：
  ./图谱/                红外精确测温图片
  ./必填信息.xlsx         数据表（工作表名："91号文模板"）
  ./91号文报告.docx        Word报告模板
  ./必填信息_已生成.xlsx   step1 输出（step2 从这里读取）
  ./91号文报告_已生成_*.docx  step2 输出
...
DOCX_OUTPUT_BASE = os.path.join(BASE_DIR, "91号文报告_已生成")  # 后缀由程序自动追加
...
def resolve_output_docx_path(suffix=None):
    """生成不冲突的 Word 输出路径。

    规则：
      - 指定 --suffix xxx → 91号文报告_已生成_xxx.docx
      - 未指定 suffix → 91号文报告_已生成.docx（若已存在则自动递增：_1, _2...）
    """
    if suffix:
        return f"{DOCX_OUTPUT_BASE}_{suffix}.docx"

    path = f"{DOCX_OUTPUT_BASE}.docx"
    if not os.path.exists(path):
        return path

    counter = 1
    while True:
        path = f"{DOCX_OUTPUT_BASE}_{counter}.docx"
        if not os.path.exists(path):
            return path
        counter += 1
...
def read_excel_output(excel_output_path):
    """从 step1 生成的 Excel 中读回完整数据（含 OCR 结果），供 step2 使用。
...
def step1_recognize_and_review():
    ...
    # step1 输出始终写入固定文件名（step2 从这里读），不递增后缀
...
def interactive_menu():
    ...
    excel_out_exists = os.path.isfile(EXCEL_OUTPUT_PATH)
    image_dir_exists = os.path.isdir(IMAGE_DIR)
    template_exists = os.path.isfile(DOCX_TEMPLATE_PATH)
    existing_reports = sorted(glob.glob(f"{DOCX_OUTPUT_BASE}*.docx"))
    ...
    if run_step2:
        print()
        default_suffix = datetime.date.today().strftime("%Y%m%d") if not existing_reports else ""
        prompt = f"  版本后缀（回车=自动编号）: "
        suffix = input(prompt).strip()
    print()

    return run_step1, run_step2, suffix
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="91号文《红外精确测温报告》自动生成程序 V2",
...
""")
        ...
        # ========= PyCharm 模式：交互式菜单 =========
        run_step1, run_step2, suffix = interactive_menu()
    ...
    # ---------- 确定 Word 输出路径 ----------
    docx_output_path = resolve_output_docx_path(suffix)
    ...
    if run_step2:
        ...
        if run_step1:
            ...
            # 只跑 step2：从已有 Excel 读取
            full_data = read_excel_output(EXCEL_OUTPUT_PATH)

        step2_generate_docx(full_data, docx_output_path)

        # 显示已有报告列表
        existing = sorted(glob.glob(f"{DOCX_OUTPUT_BASE}*.docx"))
        if len(existing) > 1:
            print(f"\n注意：当前目录下已有 {len(existing)} 份报告文件（未被覆盖）：")
            for f in existing:
