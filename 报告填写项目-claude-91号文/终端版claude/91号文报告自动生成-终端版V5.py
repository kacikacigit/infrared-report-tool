...
V4 已有功能（全部保留）：
  1. Excel和Word输出全部自动递增编号，不再交互询问版本后缀
     ...
     - 91号文报告_已生成_1.docx, 91号文报告_已生成_2.docx ...
...
文件结构（与脚本同目录）：
  ...
  ./91号文报告_已生成_N.docx     step2 输出（自动编号）
...
# ===================== Tesseract 路径自适应（开发 / PyInstaller exe 兼容）=====================
import sys as _sys
...
EXCEL_OUTPUT_BASE = os.path.join(BASE_DIR, "必填信息_已生成")  # V4: 自动编号
EXCEL_SHEET_NAME = "91号文模板"

DOCX_TEMPLATE_PATH = os.path.join(BASE_DIR, "91号文报告.docx")
DOCX_OUTPUT_BASE = os.path.join(BASE_DIR, "91号文报告_已生成")  # V4: 自动编号
