# -*- coding: utf-8 -*-
"""
91号文红外测温报告自动生成程序 — 1图3框版
======================================================

每张图3个温度框，分别识别后写入J/K/L列，日期时间右移至M/N列。
Excel自动读取第一个sheet。时间标框可选。
...
AR1_ROI_CONFIG_FILE = os.path.join(BASE_DIR, "roi_config_ar1.json")  # 保留兼容
ROI_CONFIG_FILES = [
    os.path.join(BASE_DIR, "roi_config_infrared_1.json"),
    os.path.join(BASE_DIR, "roi_config_infrared_2.json"),
    os.path.join(BASE_DIR, "roi_config_infrared_3.json"),
]
...
def get_roi(sample_image_path, config_file, prompt_text=None, window_title=None, box_label=""):
    if prompt_text is None:
        prompt_text = f"温度区域{box_label}"
    if window_title is None:
        window_title = f"框选温度{box_label}区域 (Space/Enter确认, C取消)"
    label = f"（{box_label}）" if box_label else ""
    if os.path.exists(config_file):
        with open(config_file, "r", encoding="utf-8") as f:
            cfg = json.load(f)
        rx, ry, rw, rh = cfg["rx"], cfg["ry"], cfg["rw"], cfg["rh"]
        print(f"检测到已保存的识别区域配置{label}: rx={rx:.4f}, ry={ry:.4f}, rw={rw:.4f}, rh={rh:.4f}")
        choice = input(f"是否复用该区域{label}？(y=复用 / n=重新框选) [y]: ").strip().lower()
        if choice in ("", "y", "yes"):
            return rx, ry, rw, rh

    print(f"\n请在弹出的窗口中框选{label}温度区域")
...
def read_excel_output(excel_output_path=None):
    ...
    for info in data_rows_info:
        r = info['row']
        # 3框版：J/K/L=3个温度, M=日期, N=时间
        j_val = ws.cell(row=r, column=10).value  # Ar1温度
        k_val = ws.cell(row=r, column=11).value  # Ar2温度
        l_val = ws.cell(row=r, column=12).value  # Ar3温度
        m_val = ws.cell(row=r, column=13).value  # 拍摄日期
        n_val = ws.cell(row=r, column=14).value  # 拍摄时间

        # 解析日期
        date_val = m_val
        if isinstance(m_val, datetime.datetime):
            date_val = m_val.date()
        elif isinstance(m_val, str) and m_val != NOT_RECOGNIZED_TEXT:
            parsed = parse_date(m_val)
            if parsed:
                date_val = parsed
        ...
        if isinstance(n_val, datetime.time):
            time_val = n_val.strftime("%H:%M:%S")

        # 3个温度独立存储
        def _fmt_temp(v):
            if v is None or v == NOT_RECOGNIZED_TEXT:
        ...
        row_data = dict(info)
        row_data['ar1'] = _fmt_temp(j_val)
        row_data['ar2'] = _fmt_temp(k_val)
        row_data['ar3'] = _fmt_temp(l_val)
...
def _load_image_into(label, path, ref_dict, ref_key, max_w=580, max_h=320):
    ...
    if path is None or not os.path.isabs(path):
        full = os.path.join(DEBUG_DIR, path) if path else None
    else:
        full = path
...
def interactive_review_ar1_gui(data_rows_info, ar1_results, ar1_debug_map):
    ...
    try:
        ...
        return interactive_review_cli(data_rows_info, ar1_results, ar1_debug_map)
    ...
    for i, info in enumerate(data_rows_info):
        for box_idx in range(3):
            items.append((i, box_idx))
    total_items = len(items)

    photo_refs = {}

    root = tk.Tk()
    root.title("91号文测温 - 逐框核对")
    ...
    def load_item(pos):
        row_idx, box_idx = items[pos]
        info = data_rows_info[row_idx]
        r = info['row']
        current = reviewed.get(r)  # list [v1, v2, v3] or None
        val = ""
        if current is not None and box_idx < len(current):
            v = current[box_idx]
            val = "" if v in (None, "?") else str(v)
        box_label = f"图{row_idx + 1}_框{box_idx + 1}"
        info_label.config(text=f"[{pos + 1}/{total_items}]  {box_label}  设备: {info['device_name']}")
        # 显示对应框的裁剪图
        dbg_list = ar1_debug_map.get(r, [None, None, None])
        dbg_name = dbg_list[box_idx] if dbg_list and box_idx < len(dbg_list) else None
        _load_image_into(img_label, dbg_name, photo_refs, 'crop')
        entry_var.set(val)
        hint_label.config(text="↑ = 上一个    |    ↓ = 下一个    |    回车 = 确认提交", fg="gray")
        entry.focus_set()
    ...
    def update_buttons():
        pos = state['index']
        btn_ref['prev'].config(state=(tk.DISABLED if pos == 0 else tk.NORMAL))
        btn_ref['next'].config(state=(tk.DISABLED if pos + 1 >= total_items else tk.NORMAL))

    def try_save():
        pos = state['index']
        row_idx, box_idx = items[pos]
        r = data_rows_info[row_idx]['row']
        val = entry_var.get().strip()
        # 确保该行有列表
        if reviewed.get(r) is None:
            reviewed[r] = [None, None, None]
        if val != "":
            try:
                reviewed[r][box_idx] = float(val)
            except ValueError:
    ...
    def go_prev(event=None):
        ...
        if state['index'] == 0:
            return
        state['index'] -= 1
        load_item(state['index'])

    def go_next(event=None):
        if not try_save():
            return
        if state['index'] + 1 >= total_items:
    ...
    btn_ref['prev'] = tk.Button(button_frame, text="↑ 上一个", command=go_prev, width=10)
    btn_ref['prev'].grid(row=0, column=0, padx=4)
    btn_ref['next'] = tk.Button(button_frame, text="↓ 下一个", command=go_next, width=10)
    ...
    root.bind('<Up>', go_prev)
    root.bind('<Down>', go_next)
    root.bind('<Return>', confirm_submit)

    print(f"\n已打开逐框核对窗口（共 {total_items} 个框 | ↑↓翻阅 | 回车=确认提交）...")
    load_item(0)
    root.lift()
    root.focus_force()
    entry.focus_set()
...
def step1_recognize_and_review(excel_output_path):
    ...
    roi_ratios = []
    labels = ["框1", "框2", "框3"]
    for idx, config_file in enumerate(ROI_CONFIG_FILES):
        roi_ratios.append(get_roi(sample_path, config_file, box_label=labels[idx]))
    ...
    ar1_results = {}   # 每个row存列表 [v1, v2, v3]
    date_results = {}
    time_results = {}
    ar1_debug_map = {}  # 每个row存列表 [debug1, debug2, debug3]
    dt_debug_map = {}
    ...
    for i, info in enumerate(data_rows_info, start=1):
        ...
        if img_path is None:
            ...
            ar1_debug_map[r] = [None, None, None]
            dt_debug_map[r] = None
        ...
        all_ok = True
        dbg_list = []
        for box_idx, roi_ratio in enumerate(roi_ratios):
            ar1_val, ar1_crop, ar1_raw = ocr_ar1_temperature(img_path, roi_ratio)
            values.append(ar1_val if ar1_val is not None else "?")
            if ar1_val is None:
                all_ok = False
            if SAVE_DEBUG_CROPS and ar1_crop is not None:
                name = re.sub(r'[\\/:*?"<>|]', "_", f"row{r}_box{box_idx+1}.png")
                save_img = cv2.resize(ar1_crop, None, fx=2, fy=2, interpolation=cv2.INTER_CUBIC)
                cv2.imwrite(os.path.join(DEBUG_DIR, name), save_img)
                dbg_list.append(name)
            else:
                dbg_list.append(None)
        ar1_results[r] = values if all_ok else None
        ar1_debug_map[r] = dbg_list
        ...
        if has_datetime:
            ...
            if SAVE_DEBUG_CROPS and dt_crop is not None:
        ...
        if has_datetime:
            status = f"温度={combined if all_ok else NOT_RECOGNIZED_TEXT}  " \
                     f"日期={date_val if date_val else NOT_RECOGNIZED_TEXT}  " \
                     f"时间={time_val if time_val else NOT_RECOGNIZED_TEXT}"
        else:
            status = f"温度={combined if all_ok else NOT_RECOGNIZED_TEXT}  日期/时间=已跳过"
    ...
    # 写回Excel（J/K/L=3个温度, M=日期, N=时间）
    for info in data_rows_info:
        r = info['row']
        vals = ar1_results.get(r)
        date_val = date_results.get(r)
        time_val = time_results.get(r)

        if vals is None:
            ws.cell(row=r, column=10).value = NOT_RECOGNIZED_TEXT
            ws.cell(row=r, column=11).value = NOT_RECOGNIZED_TEXT
            ws.cell(row=r, column=12).value = NOT_RECOGNIZED_TEXT
        else:
            ws.cell(row=r, column=10).value = str(vals[0]) if vals[0] is not None else NOT_RECOGNIZED_TEXT
            ws.cell(row=r, column=11).value = str(vals[1]) if len(vals) > 1 and vals[1] is not None else NOT_RECOGNIZED_TEXT
            ws.cell(row=r, column=12).value = str(vals[2]) if len(vals) > 2 and vals[2] is not None else NOT_RECOGNIZED_TEXT
        if has_datetime:
            ws.cell(row=r, column=13).value = date_val if date_val is not None else NOT_RECOGNIZED_TEXT
            ws.cell(row=r, column=14).value = time_val if time_val is not None else NOT_RECOGNIZED_TEXT
        else:
            ws.cell(row=r, column=13).value = NO_TIME_TEXT
            ws.cell(row=r, column=14).value = NO_TIME_TEXT
    ...
    for info in data_rows_info:
        ...
        if vals and len(vals) >= 3:
            row_data['ar1'] = str(vals[0]) if vals[0] not in (None, "?") else NOT_RECOGNIZED_TEXT
            row_data['ar2'] = str(vals[1]) if vals[1] not in (None, "?") else NOT_RECOGNIZED_TEXT
            row_data['ar3'] = str(vals[2]) if vals[2] not in (None, "?") else NOT_RECOGNIZED_TEXT
        else:
            row_data['ar1'] = NOT_RECOGNIZED_TEXT
            row_data['ar2'] = NOT_RECOGNIZED_TEXT
            row_data['ar3'] = NOT_RECOGNIZED_TEXT
...
def fill_table1(table, row_data):
    """1图3框版：Ar1→第3列, Ar2→第5列, Ar3→第7列"""
    row = table.rows[0]
    for col_idx, key in [(2, 'ar1'), (4, 'ar2'), (6, 'ar3')]:
        val = row_data.get(key)
        display = f"{val}℃" if val not in (None, NOT_RECOGNIZED_TEXT, "") else NOT_RECOGNIZED_TEXT
        set_cell_text(row.cells[col_idx], display)
