...
与普通版的区别：每张红外图片上有 3 个温度框，需框选 3 次，OCR 识别 3 个值。
E列写入 "v1/v2/v3" 格式，F列（正常温度）取三者最小值。

工作表：Excel 的 sheet 名为 "3个框"。
...
REPORT_DIR = os.path.abspath(os.path.join(BASE_DIR, "..", "2_report"))  # 模板和输出
...
if os.path.exists(_tesseract_exe):
    ...
    pytesseract.pytesseract.tesseract_cmd = _tesseract_exe  # V5: 自动编号
...
def get_roi(sample_image_path, config_file, box_label=""):
    """框选一个温度区域，配置按文件名独立保存（支持多个框）。"""
...
def step1_ocr_fill_surface_temperature(ws, header_row_idx, data_rows_info):
    """
    1图3框版：每张图片框选3个区域，OCR识别3个温度值。
    返回：
        surface_temp: {行号: "v1/v2/v3" 格式字符串 或 None}
        debug_file_map: {行号: 调试图片文件名 或 None}
    """
    ...
    roi_ratios = []
    labels = ["框1", "框2", "框3"]
    for i, config_file in enumerate(ROI_CONFIG_FILES):
        roi_ratios.append(get_roi(sample_path, config_file, box_label=labels[i]))
    ...
    for i, info in enumerate(data_rows_info):
        ...
        for box_idx, roi_ratio in enumerate(roi_ratios):
            ...
            if SAVE_DEBUG_CROPS and raw_crop is not None:
                debug_name = f"row{row}_box{box_idx+1}.png"
        ...
        surface_temp[row] = values if all_ok else None  # 存储为列表 [v1, v2, v3]

        status = combined if all_ok else NOT_RECOGNIZED_TEXT
        ...
        for box_idx in range(3):
            debug_name = f"row{row}_box{box_idx+1}.png"
            dbg.append(debug_name)
        debug_file_map[row] = dbg
...
def compute_normal_temperature_3box(data_rows_info, surface_temp):
    """正常温度(F) = 同行3个框温度的最小值。surface_temp 值为列表 [v1, v2, v3] 或 None。"""
    result = {}
    for info in data_rows_info:
        r = info['row']
        vals = surface_temp.get(r)
        if vals is None:
            result[r] = NOT_RECOGNIZED_TEXT
            continue
        try:
            floats = [float(v) for v in vals if v is not None and v != "?"]
            result[r] = format_number(min(floats)) if floats else NOT_RECOGNIZED_TEXT
...
def interactive_review_cli(data_rows_info, surface_temp, debug_file_map):
    ...
    for i, info in enumerate(data_rows_info, start=1):
        ...
        if current is None:
            status_display = NOT_RECOGNIZED_TEXT
            mark = "   ⚠️ 识别失败，需手动输入"
            vals = [None, None, None]
        else:
            vals = current if len(current) == 3 else (current + [None] * 3)[:3]
            status_display = "/".join(str(v) if v else "?" for v in vals)
            mark = ""
        ...
        for box_i in range(3):
            label = f"{info['atlas_code']}_框{box_i + 1}"
            while True:
                prompt = f"  {label}(当前={new_vals[box_i]}) 回车确认/s跳过/输入修正: "
                ...
                if user_input.lower() in ("s", "skip"):
                    ...
                    return reviewed
                try:
                    new_vals[box_i] = float(user_input)
                    break
                except ValueError:
                    print("  不是有效数字。")
        reviewed[r] = new_vals
...
def interactive_review_gui(data_rows_info, surface_temp, debug_file_map):
    ...
    def load_item(pos):
        ...
        box_label = f"{info['atlas_code']}_框{box_idx + 1}"
        info_label.config(text=f"[{pos + 1}/{total_items}]  {box_label}  设备: {info['device_name']}")
        # 显示对应框的裁剪图
        dbg_list = debug_file_map.get(r, [None, None, None])
...
def step1_ocr_and_review(excel_output_path):
    ...
    # 第三步：写入E列（列表→"v1/v2/v3"格式）
    for info in data_rows_info:
        r = info['row']
        vals = surface_temp.get(r)
        if vals is None:
            ws.cell(row=r, column=5).value = NOT_RECOGNIZED_TEXT
        else:
            ws.cell(row=r, column=5).value = "/".join(str(v) for v in vals)
    ...
    # 第五步：计算正常温度(F) = min(3个框温度)
    calc_result = compute_normal_temperature_3box(data_rows_info, surface_temp)

    for info in data_rows_info:
        r = info['row']
        ws.cell(row=r, column=6).value = calc_result[r]
    ...
    for info in data_rows_info:
        ...
        row_data = dict(info)
        vals = surface_temp.get(r)
        row_data['surface_temp'] = "/".join(str(v) for v in vals) if vals else None
        row_data['normal_temp'] = calc_result[r]
        row_data['env_temp'] = env_temp_val
...
def read_excel_output(excel_output_path=None):
    ...
    for info in data_rows_info:
        r = info['row']
        e_val = ws.cell(row=r, column=5).value   # 表面温度 "v1/v2/v3" 格式
...
def build_conclusion_text(full_data):
    ...
    for row_data in full_data:
        # 1图3框版：用3个温度的最大差值判断发热
        combined = str(row_data.get('surface_temp', ''))
        try:
            parts = [float(x.strip()) for x in combined.split("/") if x.strip()]
            if len(parts) >= 2:
                max_diff = max(parts) - min(parts)
            else:
                continue
        except (ValueError, AttributeError):
            continue
        if max_diff > HOT_THRESHOLD:
