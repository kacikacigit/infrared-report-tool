# -*- coding: utf-8 -*-
"""
91号文《红外精确测温报告》自动生成程序 V2
======================================================

V2 新增功能：
  1. 支持分步执行：--step1（仅识别+核对+写Excel）/ --step2（仅生成Word报告）
     不带参数则两个步骤都执行（向后兼容）
  2. 防覆盖机制：Word报告默认自动追加编号后缀（_1, _2...），
     也可通过 --suffix 手动指定
  3. step2 直接读取 step1 生成好的 Excel，不再重新 OCR，适合反复调整格式重跑

使用方法：
  python3 91号文报告自动生成-终端版V2.py              # 完整运行（step1 + step2）
  python3 91号文报告自动生成-终端版V2.py --step1       # 仅运行 step1（识别 + 核对 + 写Excel）
  python3 91号文报告自动生成-终端版V2.py --step2       # 仅运行 step2（读取已有Excel，生成Word）
  python3 91号文报告自动生成-终端版V2.py --step2 --suffix v2   # step2 + 输出文件名加_v2
  python3 91号文报告自动生成-终端版V2.py --cli         # 命令行核对模式（避免macOS tkinter崩溃）
