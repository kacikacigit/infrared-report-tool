...
"""
测温记录自动生成脚本
从 测温报告_已生成.docx 读取数据，填充 测温记录.docx 模板，生成 测温记录_已生成.docx
"""
...
REPORT_PATH = os.path.join(BASE_DIR, "测温报告_已生成.docx")
TEMPLATE_PATH = os.path.join(BASE_DIR, "测温记录.docx")
OUTPUT_PATH = os.path.join(BASE_DIR, "测温记录_已生成.docx")
...
def main():
    ...
    # 1. 读取测温报告
    print(f"\n[1/3] 读取测温报告: {REPORT_PATH}")
    if not os.path.isfile(REPORT_PATH):
        print(f"错误：找不到测温报告文件: {REPORT_PATH}")
        print("请先运行 红外测温报告自动生成-claude-v3.py 生成测温报告。")
        return
