...
def locate_label_and_wen(img):
    """一次整图 OCR，同时定位"温"字和标签左边界。

    返回 (温字box(x,y,w,h), 标签左边界x)；找不到"温"返回 (None, None)。
    把原来"找温"和"找标签左边界"的两次整图 OCR 合并成一次，避免重复 OCR 拖慢速度。
    """
    ...
    # 找标签左边界（"温"左侧 250px 内、垂直方向重叠的文本最左）
    wx, wy, ww, wh = wen_box
    label_left = wx
    for (t, x, y, w, h) in frags:
        if x < wx and wx - x < 250 and abs((y + h / 2) - (wy + wh / 2)) < wh * 2:
            label_left = min(label_left, x)

    return wen_box, label_left
...
def extract_max_temp(image_path):
    ...
    无法定位或识别不到数值时，显示图片上半张供人工核对。
    """
    img = imread_unicode(image_path)
    if img is None:
        return None, None

    marker, label_left = locate_label_and_wen(img)
    if marker is None:
